...
def import_csv_to_db(db_path, csv_path):
    conn = sqlite3.connect(db_path)
    df = pd.read_csv(csv_path)

    # CSVファイルに必要なカラムがあるか確認
    required_columns = ['name', 'student_id', 'gender']
    if not all(col in df.columns for col in required_columns):
        raise ValueError("CSVファイルには name, student_id, gender のカラムが必要です")
    ...
    if not invalid_rows.empty:
        raise ValueError(f'無効な性別値があります: {", ".join(invalid_rows["gender"].unique())}')
...
# CSVファイルからデータをインポート
try:
    import_csv_to_db('database.db', 'seito.csv')
except Exception as e:
    print(f"CSVファイルのインポートエラー: {e}")
